...
class ActQuantizer(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.bits = 16
        self.separate_massive_activation = False
        self.Q = None
    ...
    def separate(self, x, quant_x):
        x_dtype = x.dtype
        # 对于massive activation这一部分, 就不量化
        init_shape = x.shape
        x = x.reshape(-1, init_shape[-1])
        quant_x = quant_x.reshape(-1, init_shape[-1])
        # 得到旋转前的数据
        origin_x = x @ self.Q.T.to(dtype=x.dtype)
        self.free()
        self.find_params(origin_x)
        if self.bits == 16:
            return x
        elif self.sym:
            origin_quant_x = sym_quant_dequant(origin_x, self.scale, self.maxq).to(x_dtype)
        else:
            origin_quant_x = asym_quant_dequant(origin_x, self.scale, self.zero, self.maxq).to(x_dtype)
        origin_quant_error = torch.norm(origin_quant_x - origin_x, dim=-1)
        assert len(origin_quant_error.shape) == 1
        self.free()
        quant_x[origin_quant_error < 9.5] = x[origin_quant_error < 9.5]
        return quant_x.reshape(*init_shape)
    ...
    def forward(self, x):
        ...
        if self.bits == 16:
            ...
            quant_x = sym_quant_dequant(x, self.scale, self.maxq).to(x_dtype)
        else:
            quant_x = asym_quant_dequant(x, self.scale, self.zero, self.maxq).to(x_dtype)

        if self.separate_massive_activation:
            quant_x = self.separate(x, quant_x)
        return quant_x
...
def add_actquant(module, name='', layers=None):
    if layers is None:
        layers = [torch.nn.Linear, ActQuantWrapper]
