...
def parser_gen():
    ...
    parser.add_argument('--fuse_rms_norm', action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument('--rotate_mode', type=str, default='hadamard',
                        choices=['hadamard', 'random', 'householder', "hadamard_householder", "orthogonal_procrustes"])
    ...
    parser.add_argument('--save_name', type=str, default=None,
                        help='The path to save experiment data, '
                             ...
                             'The data will be saved in experiments/[model]/save_name. Default: [datetime].')
    ...
    # Householder indices path
    parser.add_argument('--indices_path', type=str)

    # calibration datasets
    parser.add_argument('--output_dir', type=str)

    # separate massive activation
    parser.add_argument('--separate_gptq', action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument('--separate_fwd', action=argparse.BooleanOptionalAction, default=False)

    parser.add_argument('--disable_qk_rotation', action=argparse.BooleanOptionalAction, default=False)

    args = parser.parse_args()

    assert args.k_bits == args.v_bits, "args.k_bits and args.v_bits must be the same"
    if args.rotate:
        quant_type = f'W{args.w_bits}A{args.a_bits}KV{args.k_bits}_{args.rotate_mode}'
    else:
        quant_type = f'W{args.w_bits}A{args.a_bits}KV{args.k_bits}'
    ...
    setattr(args, 'save_path', os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                            'experiments', args.model, quant_type, args.save_name))
